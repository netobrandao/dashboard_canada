...
with aba2:
    ...
    if countries:
        ...
        tickfont=dict(size=18),showgrid=False),yaxis=dict(showgrid=False,title="",tickfont=dict(size=16)))
...
with aba4:
    ...
    # Seleção de continentes para filtrar
    continents = st.multiselect("Selecione os Continentes:", options=continent['Continent'].unique(), 
                                default=continent['Continent'].unique())
    # Filtrar os dados
    filtered_continent = continent[continent['Continent'].isin(continents)]
    #Criar grafico de barras
    if continents:
        fig_continent = px.bar(filtered_continent,x='Continent',y='Total',
                            width=600, height=600)
...
with aba5:
    ...
    # Filtrar os dados
    filtered_devname = devname[devname['DevName'].isin(devnames)]

    if devnames:
        fig_devname = px.bar(filtered_devname,x='DevName',y='Total',width=600, height=600)
